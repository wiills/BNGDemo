...
bool UExQuestManagerSubsystem::ApplyObjectiveProgressFloat(FExQuestTask& Task, const FGameplayTag& ObjectiveTag, float NewProgress)
{
	for (FExQuestObjective& Objective : Task.Objectives)
	{
		if (Objective.ObjectiveTag != ObjectiveTag)
		...
		const bool bWasCompleted = Objective.State == EExQuestState::Completed;
		Objective.CurrentProgressFloat = FMath::Clamp(NewProgress, 0.f, static_cast<float>(Objective.TargetProgress));
		Objective.CurrentProgress = FMath::Clamp(FMath::FloorToInt(Objective.CurrentProgressFloat), 0, Objective.TargetProgress);
		Objective.ApplyProgressToState();
		OnQuestObjectiveUpdated.Broadcast(Objective);
		if (Objective.State == EExQuestState::Completed && !bWasCompleted)
		{
...
bool UExQuestManagerSubsystem::UpdateQuestObjectiveFloat(const FGameplayTag& TaskId, const FGameplayTag& ObjectiveTag, float NewProgress)
{
	const bool bResult = FindAndUpdateTask(TaskId, [this, &ObjectiveTag, NewProgress](FExQuestTask& Task) -> bool
	{
		return ApplyObjectiveProgressFloat(Task, ObjectiveTag, NewProgress);
	});
	return bResult;
}
...
bool UExQuestManagerSubsystem::IncrementQuestObjectiveFloat(const FGameplayTag& TaskId, const FGameplayTag& ObjectiveTag, float Delta)
{
	FExQuestTask Task;
	if (!CurrentQuestData.FindTaskById(TaskId, Task))
	{
	...
	for (const FExQuestObjective& Objective : Task.Objectives)
	{
		if (Objective.ObjectiveTag == ObjectiveTag)
		{
			return UpdateQuestObjectiveFloat(TaskId, ObjectiveTag, Objective.CurrentProgressFloat + Delta);
		}
	}
	return false;
}
...
void UExQuestManagerSubsystem::CreateLatentTaskForQuest(const FGameplayTag& TaskId)
{
	...
	UExLatentTask_Quest* LatentTask = UExLatentTask_Quest::CreateQuestProxy(World, Task.QuestTaskClass);
	if (!LatentTask)
	{
		UE_LOG(LogBlueprintNodeGraph, Warning,
...
void UExQuestManagerSubsystem::CreateLatentTaskForObjective(const FGameplayTag& TaskId, const FGameplayTag& ObjectiveTag)
{
	...
	if (!Objective || !Objective->GetQuestTaskClass())
	{
		return;
	}
	...
	UExLatentTask_Quest* LatentTask = UExLatentTask_Quest::CreateQuestProxy(World, Objective->GetQuestTaskClass());
	if (!LatentTask)
	{
		UE_LOG(LogBlueprintNodeGraph, Warning,
	...
	LatentTask->CompleteAction = Objective->GetCompleteAction();
	LatentTask->ProgressDelta = Objective->GetProgressDelta();
