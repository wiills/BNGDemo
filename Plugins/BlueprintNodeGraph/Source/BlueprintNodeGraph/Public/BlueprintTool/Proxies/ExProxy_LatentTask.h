...
#include "ExProxy_LatentTask.generated.h"

/**
 * @class UExProxy_LatentTask
 * @brief 创建延迟任务的代理类
 * 
 * 由 ExK2Node_LatentTaskObject 调用，用于创建延迟任务实例。
 */
UCLASS(HideDropdown, meta = (ExposedAsyncProxy = AsyncTask, SafeHideThen))
class BLUEPRINTNODEGRAPH_API UExProxy_LatentTask : public UExBase_LatentTask
{
	GENERATED_BODY()
...
public:
	...
	static UExProxy_LatentTask* CreateProxy(UObject* WorldContextObject, TSubclassOf<UExProxy_LatentTask> Class);
};

/**
 * @class UExProxy_LatentTaskUUID
 * @brief 带UUID唯一标识的延迟任务代理类
 * 
 * 支持多分支同步，等待所有输入分支完成后执行输出。
 */
UCLASS(HideDropdown, meta = (ExposedAsyncProxy = AsyncTask, SafeHideThen))
class BLUEPRINTNODEGRAPH_API UExProxy_LatentTaskUUID : public UExBase_LatentTask
{
	GENERATED_BODY()
...
public:
	...
	static UExProxy_LatentTaskUUID* CreateProxy(UObject* WorldContextObject, TSubclassOf<UExProxy_LatentTaskUUID> Class, FString UUID, int32 InputCount);
	...
	{
		UE_LOG(LogLatentTask, Display, TEXT("[UExProxy_LatentTaskUUID::Activate] - %s, Count: %d"), *GetName(), m_InputBranchCount);
		m_InputBranchCount--;
